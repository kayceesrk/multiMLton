      ...
      if (i == argc)
        ...
        if (0 == strcmp (arg, "copy-generational-ratio")) {
          ...
          *worldFile = argv[i++];
          ...
          size_t bytes = stringToBytes (argv[i++]);
          fprintf (stderr, "BYTES = %ld\n", bytes);
          size_t maxHeapLocal = align (bytes, 2 * s->sysvals.pageSize);
          fprintf (stderr, "maxHeapLocal = %ld\n", maxHeapLocal);
          s->controls->maxHeapLocal = maxHeapLocal;
