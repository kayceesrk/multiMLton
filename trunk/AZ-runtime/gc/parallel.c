...
mutex_t *waitMutex;
condition_t *waitCondVar;
bool* dataInMutatorQ;

void Parallel_initResources (GC_state s) {
  Parallel_mutexes = (int32_t *) malloc (s->numberOfProcs * sizeof (int32_t));
  waitMutex = (mutex_t*) malloc (s->numberOfProcs * sizeof (mutex_t));
  waitCondVar = (condition_t*) malloc (s->numberOfProcs * sizeof (condition_t));
  dataInMutatorQ = (bool*) malloc (s->numberOfProcs * sizeof(bool));
  for (int proc = 0; proc < s->numberOfProcs; proc++) {
    Parallel_mutexes[proc] = -1;
    mutex_init (&waitMutex[proc]);
    condition_init (&waitCondVar[proc]);
...
void Parallel_init (void) {
  GC_state s = thread_get_specific (gcstate_key);
  ...
  */
  ...
  AGAIN:
    ...
    if (Parallel_mutexes[p] >= 0)
      goto AGAIN;
  } while(atomic_compare_and_exchange_word(myNumber, (volatile)&Parallel_mutexes[p], -1) != -1);
  /*
  if (needGCTime (s))
...
void Parallel_unlock (Int32 p) {
  ...
  //fprintf (stderr, "unlock %d\n", Parallel_holdingMutex);
    ...
    if (atomic_compare_and_exchange_word(-1, &Parallel_mutexes[p], myNumber) != myNumber) {
...
Int32 Parallel_fetchAndAdd (pointer p, Int32 v) {
  ...
  volatile Int32_t *q = p;
  Int32_t res = atomic_add_32(v, q) - v;
...
bool Parallel_compareAndSwap (pointer p, Int32 old, Int32 new) {
    int x = atomic_compare_and_exchange_word(new, (Int32 *)p, old);
    if(x == old)
      return true;
    else
      return false;
...
Int32 Parallel_vCompareAndSwap (pointer p, Int32 old, Int32 new) {
      return atomic_compare_and_exchange_word(new, (Int32 *)p, old);
}
