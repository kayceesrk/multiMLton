...
size_t sizeofHeapDesired (GC_state s, size_t liveSize, size_t currentSize, GC_heapKind kind) {
  ...
  if (withMapsRatio >= s->controls->ratios.live + s->controls->ratios.grow) {
    ...
    size_t temp = (size_t)s->controls->ratios.live;
    res = liveSize * temp;
...
void releaseHeap (GC_state s, GC_heap h) {
  ...
  GC_release (h->start - h->offset, h->withMapsSize);
  initHeap (s, h, h->kind);
}
...
bool createHeap (GC_state s, GC_heap h,
  ...
  do {
    ...
    for (i = 1; i <= count; i++) {
      ...
      newStart = GC_mmapAnon ((pointer)address, newWithMapsSize + s->sysvals.pageSize);
      unless ((void*)-1 == newStart) {
        direction = not direction;
        size_t start = (size_t)newStart;
        start = ((start + s->sysvals.pageSize - 1)/s->sysvals.pageSize) * s->sysvals.pageSize;
        h->start = (pointer) start;
        h->offset = h->start - newStart;
        if (DEBUG or s->controls->messages)
          fprintf (stderr, "mallocStart "FMTPTR" start "FMTPTR" offset "FMTPTR"\n",
                  (uintptr_t)newStart, (uintptr_t)h->start, (uintptr_t)h->offset);
        h->size = newSize;
