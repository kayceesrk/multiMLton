...
void runProfHandler (void* arg) {                                       \
        ...
        thread_set_specific (gcstate_key, s);                           \
                                                                        \
        /* Block all signals */                                         \
        /*sigset_t blockSet; */                                         \
        /*sigfillset (&blockSet);*/                                     \
...
void run (void *arg) {                                                  \
        ...
        /* Save our state locally */                                    \
        thread_set_specific (gcstate_key, s);                           \
        thread_self (&s->pthread);   /*Pointer or not??*/	        \
                                                                        \
...
PUBLIC int MLton_main (int argc, char* argv[]) {                        \
        ...
        for (procNo = 1; procNo < gcState[0].numberOfProcs; procNo++) { \
                if (thread_create_and_init ((0x10000), run, (void *)&gcState[procNo], &threads[procNo - 1])) { \
                        fprintf (stderr, "pthread_create failed: %s\n", strerror (errno)); \
                        exit (1);                                       \
                }                                                       \
        }                                                               \
        for (procNo = 1; procNo < gcState[0].numberOfProcs; procNo++) { \
		int i = thread_resume(threads[procNo-1]);	\
        }								\
