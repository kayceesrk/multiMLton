...
structure TSP : sig
    ...
    val tsp : (Tree.tree * int * int) -> Tree.tree
  ...
  (* Compute TSP for the tree t -- use conquer for problems <= sz * *)
    fun tsp (t as T.ND{left, right, sz=sz', ...}, sz, spawnSz) =
          if (sz' <= sz)
            then conquer t
          else if (sz' > spawnSz) then
            let
              val (ch1, ch2) = (channel (), channel ())
              val _ = spawn (fn () => send (ch1, tsp(left, sz, spawnSz)))
              val _ = spawn (fn () => send (ch2, tsp(right, sz, spawnSz)))
            in
              merge (recv ch1, recv ch2, t)
            end
          else
            merge (tsp(left, sz, spawnSz), tsp(right, sz, spawnSz), t)
      | tsp (T.NULL, _, _) = T.NULL
...
structure Main : sig
    ...
    fun doit (n, spawnSz) = ignore (TSP.tsp (mkTree n, !divideSz, spawnSz))

  end

val (problemSz, spawnSz) =
...
val _ = TextIO.print (concat ["problemSz = ", Int.toString problemSz, "\n"])
val _ = TextIO.print (concat ["spawnSz = ", Int.toString spawnSz, "\n"])
val _ = MLton.Pacml.run (fn () => Main.doit (problemSz, spawnSz))
val te = Time.now ()
val d = Time.-(te, ts)
