...
structure TSP = struct
  ...
  (* Merge two cycles as per Karp *)
    ...
    let
      val ch = channel()
      val () = print (concat [Int.toString sz, "\n"])
      fun work () =
        (let
          val returnTree =
            (case t of
                 T.NULL => T.NULL
               | T.ND{left, right, sz=sz', ...}=>
                    if (sz' <= sz)
                        then conquer t
                        else (let
                                val leftCh = tsp(left, sz)
                                val rightCh = tsp(right, sz)
                                val arg = (combine(leftCh, rightCh, t))
                              in
                                merge arg
                              end
                              )
