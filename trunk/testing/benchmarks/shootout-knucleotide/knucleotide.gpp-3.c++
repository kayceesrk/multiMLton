...
tcalculate(const std::string& input,unsigned size)
{
   ...
   std::future<__gnu_pbds::cc_hash_table<T,unsigned,T::hash>> ft[NUM_PROC];
   for(unsigned i = 0; i<NUM_PROC;++i)
      ft[i] = std::async(std::launch::async,calculate,std::ref(input),size,i,NUM_PROC);

   auto frequencies = ft[0].get();

   for(unsigned i = 1 ; i<NUM_PROC; ++i)
      for(auto& j : ft[i].get())
      {
