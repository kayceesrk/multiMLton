	...
	for b in benchmarks:
		for n in numProcs:
			...
			for i in range (0, totalRuns):
				...
				ml = common.bytesIntToString (mlInt, 1)
				ms = common.bytesIntToString (msInt, 1)
				...
				if (shouldRun):
					...
					for i in range(0, reruns):
						(_r, _m, _mlr, _msr) = common.run ("./" + str(b), str(progName[b]), atMLtons, args[b])
						if int(_r) == 0:
							failed += 1
					...
					c.execute ('insert into runTime values (?, ?, ?, ?, ?, ?, ?, ?)', \
										(b, n, common.bytesIntToString (mlr, 1), common.bytesIntToString (msr, 1), common.bytesIntToString (m, 1), "WB", args[b], int(r)))
