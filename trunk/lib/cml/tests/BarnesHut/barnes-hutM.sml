...
functor Main (V : VECTOR) : sig
  ...
  fun work(plist, start, space, dtime, eps, nstep, rmin, tnow, tol) =
  let
  ...
  (* advance bodies *)
    ...
    val (n2bcalc,nbcalc,selfint) = if (start + chunkSize > (Array.length plist))
                                   then
                                     recalc((Array.length plist) - 1 - start, 0, 0, 0)
                                   else
                                     recalc(chunkSize - 1,0,0,0)
    val slice = if (start + chunkSize > (Array.length plist)) then
                  ArraySlice.slice (plist, start, NONE)
                else
                  ArraySlice.slice (plist, start, SOME chunkSize)
    val _ = ArraySlice.app advance slice
  in 0
  end
  ...
  fun slave1 (plist, workChan,resultChan,id,space,dtime,eps,nstep,rmin,tnow,tol) =
	let
	val (gg, start) = recv workChan
	val _ = case gg of
               DONE => ()
             | WORK =>
			let
	 		val result = work (plist, start, space, dtime, eps, nstep, rmin, tnow, tol)
			val _ = spawn(fn () => (send(resultChan, result)))
			val _ = slave1 (plist, workChan,resultChan,id,space,dtime,eps,nstep,rmin,tnow,tol)
			in () end
	in () end


  fun master1 (plist, dtime, eps, nstep, rmin, rsize, tnow, tol) =
	let
    ...
    fun splitWork i =
		if i > totalWork then ()
			...
			let
			val _ = spawn(fn () => send(workChan,(WORK, i)))
			in splitWork (i+chunkSize) end
      val _ = splitWork 0

      fun assignWork j =
		if j > totalSlaves then ()
		else (spawn(fn() => slave1 (plist, workChan,resultChan, j,s,dtime,eps,nstep,rmin,tnow,tol)) ; assignWork(j+1))
		...
		val _ = if i > totalWork
			then    (let
				fun loop i = if i > totalSlaves then
                              ()
                             else (spawn(fn ()=> send(workChan,(DONE,3)))
                                   ; loop(i+1))
				val _ = loop 0
				in () end )
			else let val _ = () (*print "\nRecv Result"*) val resultChunk = recv resultChan in recvResult(resultChan, i+chunkSize) end
		in () end
  ...
  (* advance N-body system one time-step. *)
    ...
    let
      val _ = master1 (plist, dtime, eps, nstep, rmin, rsize, tnow, tol)
    in
        (nstep+1, tnow + dtime)
    end
...
structure Main : BMARK =
  struct
    ...
    in
        MLton.RunPCML.doit(fn()=>(M3.doit() ), NONE); ()
    end
