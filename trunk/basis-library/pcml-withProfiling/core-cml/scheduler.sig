...
signature SCHEDULER =
   sig
      ...
      type statType = RepTypes.statType
      ...
      val getStats : unit -> statRec
      val setStats : statRec -> unit
      val incStat : statType -> unit
      val statCopyParasite : int -> unit
      val writeGlobalStats : unit -> unit
      val writeMainThreadStats : unit -> unit
      val outstrm : TextIO.outstream option ref

      val async : (unit -> unit) -> unit
      val atomicPrefixAndSwitchTo : Primitive.MLton.Thread.thread * statRec -> unit
