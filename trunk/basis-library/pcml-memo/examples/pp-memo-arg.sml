
structure Main =
struct

  open MLton
  open PCML

   fun p () : string =
        (*Int.toString(B.processorNumber())*)""

  fun print' f = print f
   fun print f = ""
   fun pong ch =
      let
        fun bar () =
        let
                    val _ = print ("\nReceiving "^(p()))
                    val i = (recv ch)
                    val _ = print ("\nReceived "
                                    ^" "^(p()))
        in
          ()
        end

        val bar = Stable.stable bar

         fun loop () =
            let
              val _ = bar ()
             in
               loop ()
            end
         val _ = print ("\nSpawning pong"^(p()))
         val _ = spawn (fn () => (print ("\nRunning pong "^(p())); loop ()))
      in
         ()
      end

   fun ping ch n str =
      let
      fun foo () = let val _ = print ("\nSending "^(p())
                          ^" "^Int.toString(5))
            val () = (send (ch, str))
            val _ = print ("\nSent "^(p())
                          ^" ")
      in
        ()
      end

  val foo = Stable.stable foo


         fun loop i =
            if i > n then RunPCML.shutdown OS.Process.success
               else let
                 val _ = foo ()
                    in
                       loop (i + 1)
                    end
         val _ = print ("\nSpawning ping"^(p()))
         val _ = spawn (fn () => (print ("\nRunning ping "^p());loop 0))
      in
         ()
      end

   fun doit (n, str) =
   let
     val _ = print' ("\nIn doit"^str)
   in
      RunPCML.doit
      (fn () =>
       let
          val _ = print ("\nCreating channel "^(p()))
          val ch = channel ()
          val _ = print ("\nCreating pong "^(p()))
          val () = pong ch
          val _ = print ("\nCreating ping "^(p()))
          val () = ping ch n str
         val _ = print ("\nCreated ping and pong "^(p()))
       in
          ()
       end,
       SOME (Time.fromMilliseconds 10))
       (* n = 2,000,000 *)
   end
end

val (n,len) =
   case CommandLine.arguments () of
      [] => (100, 10)
    | s::(l::_) => (case (Int.fromString s, Int.fromString l) of
                  (NONE, NONE) => (100, 10)
                | (SOME n, NONE) => (n, 10)
                | (SOME n, SOME len) => (n, len))

fun mkString (i, str) = if i>0 then mkString(i-1, str^"x") else str


val ts = Time.now ()
val _ = TextIO.print "\nStarting main"
val _ = Main.doit (n, mkString(len, ""))
val te = Time.now ()
val d = Time.-(te, ts)
val _ = TextIO.print (concat ["Time start: ", Time.toString ts, "\n"])
val _ = TextIO.print (concat ["Time end:   ", Time.toString te, "\n"])
val _ = TextIO.print (concat ["Time diff:  ", LargeInt.toString (Time.toMilliseconds d), "ms\n"])
