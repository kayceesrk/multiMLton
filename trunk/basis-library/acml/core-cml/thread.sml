   ...
   struct
      ...
      fun generalExit (tid', clr') =
         ...
         in
           if S.finishWork () then
             ...
             let
               val _ = debug' "Quiting"
               val _ = SH.pauseHook := (fn _ => (ignore (B.return ()); !SH.pauseHook(0)))
             in
      ...
      fun spawnOnProc (f, procNum) =
         let
            ...
            val t = S.new thread
            val tid = S.getThreadId t
            val () = S.readyOnProc (S.prep t, procNum)
            val () = S.atomicEnd ()
            val () = debug (fn () => concat ["spawnOnProc ", tidToString tid])  (* NonAtomic *)
         in
            tid
         end
