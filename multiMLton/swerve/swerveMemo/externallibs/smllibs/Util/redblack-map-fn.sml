...
functor RedBlackMapFn (K : ORD_KEY) :> ORD_MAP =
  struct
   ...
   *)
	 ...
	 *)
	  ...
	  fun del (E, z) = raise LibBase.NotFound
	    | del (T(color, a, yk, y, b), z) = (case K.compare(k, yk)
		 of LESS => del (a, LEFT(color, yk, y, b, z))
		  | EQUAL => (y, join (color, a, b, z))
		  | GREATER => del (b, RIGHT(color, a, yk, y, z))
		(* end case *))
