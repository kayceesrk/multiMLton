      ...
      *)
      ...
      fun channel () =
        let
            ...
            val taking_memo = !taking_memo
            val _ = if taking_memo
                    then let  val con = ref (fn x => (print(" I shouldnt see this! [CHANNEL]\n");x))
            	    	      val cpyF = fn h =>
                    	            C.callcc(fn z =>(con:= (fn x => (ignore (C.throw(z, x)); x)); h))
                         in (cpyF(); memo_cache := ((fn () => !con()) o !memo_cache))
                         end
                    else ()
         ...
         let
            fun doitFn () =
               let
                  ...
                  (* KC send code for memoization
                  *  This definition of tryLp overwrites previous definition *)
                  ...
                  val () = case ret of
                              ...
                              | SOME _ => (* Update memo cache only when doitFn is successful *)
                                  let
                                    val _ = if taking_memo
                                            then memo_cache := ((fn () =>
                                                  let
                                  ...
                                  in
                                    Assert.assertNonAtomic' "Channel.sendEvt(3.1.4)"
                                  end
               in
                 ret
               end
      ...
      fun recv (CHAN {prio, inQ, outQ, lock}) =
         ...
         in
            (if taking_memo
            then let val con = ref (fn x => (print(" I shouldnt see this!\n");x))
                      val cpyF = fn h =>
                            C.callcc(fn z =>(con:= (fn x => (ignore (C.throw(z, x)); x)); h))
                      (* For memoization *)
                      fun tryLp () =
                        (case cleanAndDeque (outQ) of
                                      ...
                                      (let
                                          ...
                                          val msg =
                                            ...
                                            (fn rt =>
                                              ...
                                              ; debug' "Releasing lock"
                                              ; L.releaseCmlLock lock (S.tidNum ())
                                              ; S.readyOnProc (S.prepVal (st, (
                                                  S.prepend(rt,
                                                                fn x =>
                                                                    if
                                                                    Primitive.MLton.equal(x, msg)
                                                                    then ()(*print("memo sucess\n")*)
                                                                    else (
                                                                    (*print("throwing
                                                                    * to
                                                                    * con\n");*)ignore (!con(x)); ()))
                          ...
                          | NONE =>
                              let
                                  ...
                                  val msg =
                                    S.atomicSwitchToNext
                                    (fn rt => (enqueAndClean (inQ, (TransID.mkTxId (),
                                                          S.prepend(rt,
                                                                          fn x =>
                                                                        if
                                                                        Primitive.MLton.equal(x, msg)
                                            ...
                                            val msg =
                                              ...
                                              (fn rt => (enqueAndClean (inQ, (TransID.mkTxId (),
                                                                    S.prepend(rt,
                                                                                  ...
                                                                                  Primitive.MLton.equal(x, msg)
                                                                                  then ()
                                                                                  else (L.releaseCmlLock lock (S.tidNum ())
                                                                                        ;ignore (!con(x)); ()))
                                              , curProcNum))
                                              ; debug' "In atomicSwitchToNext : After recv(3.2.1)"
                                              ; debug' "Releasing lock"
