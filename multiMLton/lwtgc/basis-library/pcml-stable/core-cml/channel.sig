(* channel.sig
 * 2004 Matthew Fluet (mfluet@acm.org)
 *  Ported to MLton threads.
 *)

(* channel-sig.sml
 *
 * COPYRIGHT (c) 1995 AT&T Bell Laboratories.
 * COPYRIGHT (c) 1989-1991 John H. Reppy
 *
 * The representation of synchronous channels.
 *)

signature CHANNEL =
   sig

      type 'a chan

      val channel : unit -> 'a chan
      val sameChannel : ('a chan * 'a chan) -> bool

      val sameChannelG : ('a chan * int ref) -> bool
      val mergeChannel : (int * ThreadID.thread_id list) -> unit

      val send : ('a chan * 'a) -> unit
      val recv : 'a chan -> 'a

      val sendEvt  : ('a chan * 'a) -> unit EventType.event
      val recvEvt  : 'a chan -> 'a EventType.event

      val sendPoll : ('a chan * 'a) -> bool
      val recvPoll : 'a chan -> 'a option

      val isEmpty : ('a chan) -> bool
  end

signature CHANNEL_EXTRA =
   sig
      include CHANNEL
      (*val resetChan : 'a chan -> unit*)
   end
