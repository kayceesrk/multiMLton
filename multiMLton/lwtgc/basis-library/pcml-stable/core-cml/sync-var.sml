...
structure SyncVar : SYNC_VAR_EXTRA =
   struct
      ...
      datatype thread_id = datatype ThreadID.thread_id
      ...
      fun stableRestore(value) =
        let val origVal = !value
            val b = fn() => value := origVal
            val () = StableGraph.addSyncVarWrite(Thread.getTid(), b)
        in ()
        end
      fun stableRead(value) =
        case !value
          of NONE => raise Put
           | SOME (writeTid, _) => StableGraph.addSyncVarCom(Thread.getTid(), writeTid)
               ...
               case !value of
                  NONE =>
                     let
            			...
            			val () = stableRestore(value)
		            	val () = value := SOME (Thread.getTid(), x)
                        val () = prio := 1
         ...
         let
            fun doitFn () =
               let
                  ...
                  val res = case x of
                                NONE => (Assert.assertAtomic (fn () => concat ["SyncVar.", name, "(3.2.2)"], SOME 1); NONE)
                              | SOME (tid, v) => (Assert.assertNonAtomic (fn () => concat ["SyncVar.", name, "(3.2.2)"]); SOME v)
               in
                 res
               end
      ...
      local fun doGetSwap value = (stableRead value)
      in
      ...
      fun mVarInit x = CELL {prio = ref 0, readQ = Q.new(), value = ref (SOME
        (Thread.getTid (), x)), lock = L.initCmlLock ()}
      val sameMVar = sameCell

      fun mPut (cell, x) = gPut ("mPut", cell, x)
      local fun doTakeSwap value = (stableRestore (value); value := NONE)
      in
      ...
      local fun doSwapSwap x value = (stableRead value; stableRestore value; value := SOME (Thread.getTid(), x))
      in
