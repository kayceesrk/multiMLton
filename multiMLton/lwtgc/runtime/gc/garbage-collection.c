...
void ensureHasHeapBytesFreeAndOrInvariantForMutator (GC_state s, bool forceGC,
  ...
  /* XXXGC KC - This might be wrong. We do not want to do force a major
   * collection for the sake of handling signals. */
  if ( /* we have signals pending */
    ...
    ENTER0 (s);
    /* XXXGC KC - This is what worries me. Is there a way to do it without
     * acquiring the global lock?? */
    if (fromGCCollect and (not s->signalsInfo.amInSignalHandler)) {
        if (DEBUG or s->controls->messages)
            fprintf (stderr, "GC: switchToSignalHandlerThreadIfNonAtomicAndSignalPending [%d]\n",
                     s->procStates ? Proc_processorNumber (s) : -1);
